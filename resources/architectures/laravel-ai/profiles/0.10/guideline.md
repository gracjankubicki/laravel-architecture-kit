Compatibility profile: `laravel-ai@0.10` (`>=0.10.0 <0.11.0`).

Purpose:
Laravel AI represents production AI workflows built on `laravel/ai` 0.10.
...
- Configure provider-specific behavior through `withProviderOptions()` when the workflow requires it.
...
- Approval resumes are idempotent by tool call ID. Concurrent resumes and external side effects remain application responsibilities.
...
- Existing applications that published older conversation migrations must explicitly evaluate the `participant_type`, `participant_id` and `approval_state` schema changes before enabling remembered approvals.
...
- Tests must cover Prompt Data, Result Data validation, Gateway fakes, participant authorization, approval decisions and Tool boundaries without real providers.
