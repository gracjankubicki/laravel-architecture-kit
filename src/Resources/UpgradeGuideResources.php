...
final readonly class UpgradeGuideResources
{
    ...
    public function skills(array $enabled): array
    {
        ...
        foreach ($this->files->allFiles($root) as $file) {
            ...
            $key = 'upgrade:'.$guide['package_slug'].':'.$guide['from'].'-to-'.$guide['to'];
            ...
            if (isset($skills[$key])) {
                throw new RuntimeException("Duplicate Architecture Kit upgrade guide [{$key}].");
            }

            $names[$guide['name']] = true;
            $skills[$key] = new GeneratedFile(
                path: $this->projectPath.'/.ai/skills/'.$guide['name'].'/SKILL.md',
    ...
    /**
     * @return array{name: string, architecture: string, package_slug: string, from: string, to: string, contents: string}
     */
    private function parse(string $root, SplFileInfo $file): array
    {
