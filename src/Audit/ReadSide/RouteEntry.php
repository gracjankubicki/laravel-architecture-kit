...
final readonly class RouteEntry
{
    ...
    public static function fromRoute(Route $route): self
    {
        ...
        return new self($route->methods(), $route->uri(), $route->getName(), $route->getDomain(), $route->wheres, $class, $method, $class === null ? 'Closure or unresolved route handler.' : null);
    }
    ...
    {
        if (! is_array($data) || array_keys($data) !== ['verbs', 'uri', 'name', 'domain', 'constraints', 'class', 'method', 'unresolved']
