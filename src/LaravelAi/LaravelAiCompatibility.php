...
final readonly class LaravelAiCompatibility
{
    ...
    public function resolve(): LaravelAiCompatibilityResult
    {
        ...
        if ($package->section === 'require-dev') {
            return $this->fromPackage(
                ...
                'Run composer remove --dev laravel/ai && composer require laravel/ai:"^0.8 || ^0.9 || ^0.10 || ^0.11".',
            );
        }
        ...
        try {
            ...
            return $this->fromPackage($package, LaravelAiCompatibilityStatus::InvalidConstraint, 'Invalid laravel/ai constraint: '.$exception->getMessage(), 'Use ^0.8, ^0.9, ^0.10, ^0.11 or another fully supported constraint.');
        }
        ...
        if (! Intervals::isSubsetOf($candidate, $supported)) {
            return $this->fromPackage(
                ...
                'Narrow root require.laravel/ai to ^0.8, ^0.9, ^0.10, ^0.11, ^0.8 || ^0.9 || ^0.10 || ^0.11, or >=0.8 <0.12 and run composer update laravel/ai.',
            );
        }
        ...
        if ($profile === null) {
            return $this->fromPackage($package, LaravelAiCompatibilityStatus::UnsupportedVersion, 'The installed laravel/ai version has no verified Architecture Kit profile.', 'Install a supported laravel/ai 0.8.x, 0.9.x, 0.10.x or 0.11.x release.');
        }
    ...
    private function missingCapabilities(LaravelAiProfile $profile): array
    {
        ...
        $providerConnection = $this->basePath.'/vendor/laravel/ai/src/Exceptions/ProviderConnectionException.php';
        $providerConnectionContents = $this->files->isFile($providerConnection) ? $this->files->get($providerConnection) : '';
        $streamError = $this->basePath.'/vendor/laravel/ai/src/Exceptions/StreamErrorException.php';
        $streamErrorContents = $this->files->isFile($streamError) ? $this->files->get($streamError) : '';
        $promptable = $this->basePath.'/vendor/laravel/ai/src/Promptable.php';
        $promptableContents = $this->files->isFile($promptable) ? $this->files->get($promptable) : '';
        $allSource = '';
        ...
        $available = [
            ...
            'provider-connection-failover' => str_contains($providerConnectionContents, 'implements FailoverableException'),
            'stream-error-exception' => str_contains($streamErrorContents, 'class StreamErrorException'),
            'queued-jobs-dispatched-when-faked' => str_contains($promptableContents, 'InvokeAgent::dispatch')
                && str_contains($promptableContents, 'BroadcastAgent::dispatch'),
        ];
    ...
    {
        return 'Run composer require laravel/ai:"^0.8 || ^0.9 || ^0.10 || ^0.11".';
    }
}
